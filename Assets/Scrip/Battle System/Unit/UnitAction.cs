...
public class UnitAction : BaseEntiny
{
    ...
    void Update()
    {   
        if(BattleSystem.InBattleMode == false)
        {
            return;
        }
    ...
    protected override void Attack()
    {
        ...
        Set_Effect_atk(Unit_ID);

        if (currentTarget.UnitLive && range <= 2.5)
        {   
    ...
    private void Set_Effect_atk(string ID)
    {
        atkEffect = DataGraph.Get_Effect_Atk_By_ID(ID);
        if(atkEffect != null)
        {
